...
def main():
    chunksize = 100_000
    mode = 'w'
    for df_test in pd.read_csv('test.tsv', sep='\t', chunksize=chunksize):
        ...
        df_test_features = pd.DataFrame(scaler.transform(df_test_features),
                                        columns=[
                                            col.replace(f'feature_{feature_type}_', f'feature_{feature_type}_stand_')
        ...
        df_test_features[f'max_feature_{feature_type}_abs_mean_diff'] = res_df[
            f'max_feature_{feature_type}_abs_mean_diff']

        df_test_features = df_test_features[
            ['id_job'] + [c for c in df_test_features if c not in ['id_job']]]

        df_test_features.to_csv('test_proc.tsv', mode=mode, index=0)
        mode = 'a'
